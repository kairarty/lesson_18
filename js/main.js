...
function wordsList(str, subStr) {
    ...
    var arr = str
        ...
        .toLowerCase()
        .split(' ')
        .filter(s => s.indexOf(subStr) > -1);
    var res = new Set();
    arr.forEach(s => res.add(s));
    return res;
}
...
function getLocalDate(date, isSeconds, isISO) {
    ...
    if (!isISO) {
        if (!isSeconds) {
            return localDateString.substring(0, localDateString.length - 3);
        }
        return localDateString;
    } else {
        var year = localDateString.slice(6, 10);
        var month = localDateString.slice(3, 5);
        var day = localDateString.slice(0, 2);
        var hours = localDateString.slice(12, 14);
        var minutes = localDateString.slice(15, 17);
        var seconds = localDateString.slice(18, 20);
...
function getWeekDay(date) {
    ...
    return myDate.toLocaleDateString('ru-RU', {weekday: 'long'});
}


function getLocalDay(date) {
    var myDate = new Date(date);
    var weekDayNumber = myDate.getDay();
    if (weekDayNumber === 0) {
        return 7;
    }
